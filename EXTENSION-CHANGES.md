...
- **读取本页正文**：侧边栏右下角"📄 读取本页"按钮，点一下把当前标签页的正文（优先 `article`/`main` 等阅读区，回退 body，2 万字符封顶）连同你的选中文字一起随**下一条消息**发出——登录后的页面、JS 渲染的阅读器都能读，这是 `web_fetch` 抓不到的；正文作为普通文本部分进入持久消息，一次性使用、发完即清；
- 提交：`4e1e15a3`、`d4899824`、`848d2993`。
