 ...
 * embedded app frame; the newest accepted sample rides the next prompt like
 * the browser time zone does. Outside the extension no message ever arrives
 * and every prompt omits the field.
 */
...
let current: BrowserPageSample | undefined

/**
 * The newest accepted page sample for one outbound prompt.
 * @returns the sample, or undefined outside the extension panel.
 */
export function currentBrowserPage(): BrowserPageSample | undefined {
  return current
}
...
function acceptReport(event: MessageEvent): void {
  ...
  if (report.url.length > BROWSER_PAGE_URL_MAX_CHARS) return
