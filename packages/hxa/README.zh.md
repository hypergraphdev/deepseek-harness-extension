...
| 包 | ctx key | 角色 |
|---|---|---|
| [`hxa`](hxa/README.md) | `ctx.hxa` | Service Definition + hub 客户端 Provider：经 B2B REST 接口的组织级连接，外加 WebSocket ticket/URL |
| [`tool-hxa`](tool-hxa/README.md) | — | Consumer：面向模型的 `hxa_contacts` / `hxa_send` / `hxa_inbox` 工具 |
| [`hxa-inbound`](hxa-inbound/README.md) | — | Consumer：入站桥——一条 hub WebSocket 让 bot 保持在线，并为每条入站私信唤醒一个 coordinator agent |
